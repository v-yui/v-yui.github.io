...
数据属性包含一个数据值的位置，从该位置读取和写入。数据属性有4个描述其行为的特性：
...
`[[Value]]`默认值为undefined，直接定义在对象上上的属性，其它三个特性都默认为true。

更改属性的默认特性，使用**`Object.defineProperty()`**。如下：
...
多数情况下都没有必要利用 Object.defineProperty() 方法提供的这些高级功能。不过理解这些概念对理解 JavaScript对象非常有用。 
...
函数声明的重要特征就是函数声明提升 (function declaration hoisting)。

function后面不接标识符时创建的函数叫匿名函数 (anonymous function)，也叫lambda函数。匿名函数的name属性是空串。

严格模式下不能访问 arguments.callee来编写递归，也可以使用命名函数表达式来实现。如下：
...
2. 全局环境的变量对象始终存在，像函数这样的局部环境的变量对象，只在函数执行的过程中存在。
3. 创建函数时，会创建一个预先包含全局变量对象的作用域链，被保存在内部[[Scope]]属性中。 
4. 调用函数时，会为函数创建一个执行环境（execution context），再通过复制函数的[[Scope]]属性中的对象构建起执行环境的作用域链。
5. 此后又有一个使用 arguments 和其他命名参数的值来初始化函数的活动对象 (activation object) (在此作为变量对象使用) 被创建并被推入执行环境作用域链的前端。
6. 无论什么时候在函数中访问一个变量时，就会从作用域链中搜索具有相应名字的变量。
7. 函数执行完毕后，局部活动对象就会被销毁，内存中仅保存全局作用域（全局执行环境的变量对象）。 



**闭包的情况**

闭包的情况有所不同：函数中的匿名函数被调用时，其作用域链被初始化为包含外部函数的活动对象和全局变量对象。所以外部函数的活动对象不会在执行完毕后就销毁 (作用域链会被销毁)，会==留在内存中直到匿名函数被销毁后才被销毁==。过度使用闭包可能会导致内存占用过多。

==闭包保存的是整个变量对象==而不是某个特殊的变量，故闭包只能取得包含函数中任何变量的最后一个值。如下例：
...
function createFunctions(){		// 修改后
    var result = new Array(); 
    for (var i=0; i < 10; i++){    
        result[i] = function(num){  // 函数参数按值传递       
            return function(){               
                return num;             
            };         
        }(i); // 外部匿名函数立即执行
