...
1. p48页——Symbol 5-16之后——未看，未笔记；
2. 第三章/函数笔记——需修改；
3. p99——静态分配与对象池——未理解，未笔记；
4. p139——Array.from 集合/迭代器相关例——未理解，未笔记；
5. p161——合并复制修改定型数组——未理解，未笔记；
6. p171——WeakMap私有变量例——未理解，未笔记；
7. p176——定义正式集合操作——未看，未笔记；
8. p188——自定义/提前终止迭代器——未理解，未笔记；
...
Iterator API使用**` next()`**遍历iterable，每次调用返回一个**` IteratorResult`**对象，包含两个属性，表示是否还可以再次调用的done和包含下一个值的value。使用例：
